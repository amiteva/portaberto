...
describe("LocaleSwitcher", () => {
  it("opens the language menu and switches locale", async () => {
    ...
    expect(wrapper.text()).toContain("Macedonian");
    ...
    expect(wrapper.get("button.locale-switcher__button").text()).toContain(
      "MK",
    );
    expect(
      wrapper.get("button.locale-switcher__button").attributes("aria-expanded"),
    ).toBe("false");

    await wrapper.get("button.locale-switcher__button").trigger("click");
    expect(wrapper.text()).toContain("Англиски");
    expect(wrapper.text()).toContain("Македонски");
